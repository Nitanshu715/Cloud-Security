...
The practical report should include screenshots covering the important authorization states.

Recommended evidence:

| # | Screenshot | Evidence |
|---:|---|---|
| 01 | AWS console as `devuser` | Initial identity |
| 02 | EC2 authorization error | Restricted EC2 access |
| 03 | S3 bucket list | Restricted S3 access |
| 04 | `DeveloperGroup` Users tab | `devuser` membership |
| 05 | `DeveloperGroupPolicy` attached | Group policy assignment |
| 06 | Policy JSON | Identity-based permission analysis |
| 07 | New S3 bucket | Successful bucket creation |
| 08 | Upload failure | `AccessDenied` / missing object permission |
| 09 | Object access failure | Restricted object access |
| 10 | `BucketsAccessRole` active | Successful role assumption |
| 11 | bucket1 download | Role-based `GetObject` |
| 12 | Role policies | `ListAllBucketsPolicy` + `GrantBucket1Access` |
| 13 | Trust relationship | `devuser` trusted entity |
| 14 | bucket2 upload | Successful role-based upload |
| 15 | bucket2 bucket policy | Resource-based `PutObject` |
| 16 | bucket3 upload failure | Restricted identity |
| 17 | bucket3 policy | Resource-based policy analysis |
| 18 | Final bucket3 object | Final challenge result |
...
AWS Identity and Access Management (IAM) is the AWS service used to manage identities and control access to AWS resources.

### Q2. What is an identity-based policy?

A policy attached to an IAM user, group, or role that defines what actions that identity is allowed or denied to perform.

### Q3. What is a resource-based policy?

A policy attached directly to a resource that defines which principals can access that resource and what actions they can perform.

### Q4. Why could `devuser` create an S3 bucket but not upload an object?

Because the applicable policy allowed bucket creation but did not grant the object-upload permission `s3:PutObject`.

### Q5. What is an IAM role?

An IAM role is an AWS identity that can be assumed by a trusted principal to obtain temporary permissions.

### Q6. What is a trust relationship?

A trust relationship defines which principals are allowed to assume an IAM role.

### Q7. Why did downloading `Image2.jpg` succeed after assuming `BucketsAccessRole`?

Because the role's `GrantBucket1Access` policy grants `s3:GetObject` and related permissions for `bucket1`.

### Q8. Why did uploading to `bucket2` succeed?

Because the resource-based policy attached to `bucket2` explicitly grants `BucketsAccessRole` the `s3:PutObject` action.

### Q9. Does `GrantBucket1Access` grant `s3:PutObject`?

No.

### Q10. What does `s3:GetObject` do?

It permits retrieval of an object from Amazon S3.

### Q11. What does `s3:PutObject` do?

It permits uploading or writing an object to Amazon S3.

### Q12. Why does switching roles change permissions?

Because the active AWS principal changes and a different set of identity-based and resource-based policies becomes applicable.

### Q13. What is least privilege?

Providing only the permissions required for a specific task or responsibility.

### Q14. Why should both identity and resource policies be inspected during authorization troubleshooting?

Because access may depend on policies attached to the caller as well as policies attached directly to the target resource.
