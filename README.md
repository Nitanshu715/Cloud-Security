...
The complete laboratory now covers five experiments:
...
05 → AWS KMS Data-at-Rest Encryption
...
| **05** | **Encrypting Data at Rest by Using AWS KMS** | Encryption, key management, data-at-rest protection | KMS, S3, EBS, EC2, CloudTrail, SSE-KMS | ✅ Completed |
...
**Focus:** Encryption, customer managed keys, S3 SSE-KMS, authenticated access, CloudTrail KMS activity, EBS encryption, and KMS key lifecycle.
 
 
📁 `experiment-5/`
 
 
The fifth experiment extends the Cloud Security laboratory into **data-at-rest protection and centralized cryptographic key management**.
 
 
It demonstrates how AWS KMS can protect stored data across Amazon S3 and Amazon EBS, how authenticated access is required for SSE-KMS protected objects, how KMS operations appear in CloudTrail, and how disabling a KMS key affects dependent encrypted resources.
...
A customer managed symmetric KMS key is created in AWS Key Management Service.
...
The key provides a centrally managed cryptographic control that can be used by multiple AWS services.
 
 
---
 
 
## 🪣 Task 2 — Encrypt an S3 Object Using SSE-KMS
...
is uploaded to the lab S3 bucket using server-side encryption with an AWS KMS key.
...
The object is therefore protected using a customer managed KMS key instead of relying only on unencrypted object storage.
 
 
---
 
 
## 🌐 Task 3 — Public Access and SSE-KMS Behavior
 
 
The original S3 Object URL was tested before and after configuring public access.
 
 
The bucket was configured to allow public access through the required S3 settings and the object was made public using its ACL.
 
 
However, the original unsigned Object URL returned:
...
This demonstrates an important distinction between **object-level public accessibility** and the authenticated cryptographic operation required to use an SSE-KMS protected object.
...
## 🔗 Task 4 — Signed Access to the Encrypted Object
 
 
The encrypted object was opened through the AWS S3 console.
 
 
The resulting request contained signed parameters such as:
...
The object opened successfully through the authenticated console workflow.
 
 
---
 
 
## 🛰️ Task 5 — KMS Activity in AWS CloudTrail
 
 
AWS CloudTrail Event history was used to inspect KMS activity associated with the encrypted object.
 
 
The event source was filtered to:
...
This provides an audit trail of cryptographic API activity.
...
The EC2 instance `LabInstance` initially used an unencrypted root EBS volume.
 
 
A snapshot of the unencrypted root volume was created and then used to create a new encrypted EBS volume.
...
The new encrypted volume was created in the same Availability Zone as the original root volume.
 
 
The original unencrypted root volume was detached and the new encrypted volume was attached to `LabInstance` as:
...
The experiment then tested the effect of disabling the KMS key used to protect the encrypted resources.
...
Start fails     KMS.DisabledException
 
```
 
 
While the key was disabled:
 
 
- `LabInstance` could not successfully start.
- Access to the SSE-KMS encrypted S3 object failed with `KMS.DisabledException`.
- CloudTrail activity was inspected for the key-disable and subsequent operations.
 
 
The following CloudTrail events were examined:
...
After the test, `MyKMSKey` was re-enabled.
...
`LabInstance` was started successfully after the key was re-enabled and reached the **Running** state.
 
 
---
 
 
## 🔐 Experiment 05 Security-Control Matrix
 
 
| Control | Scope | Purpose | Demonstrated |
|---|---|---|---|
| **AWS KMS** | Cryptographic key management | Centralized encryption-key control | ✅ |
| **Customer Managed Key** | KMS | User-controlled encryption key | ✅ |
| **SSE-KMS** | Amazon S3 | Data-at-rest encryption | ✅ |
| **Signed Access** | S3 object access | Authenticated access to protected object | ✅ |
| **CloudTrail** | API auditing | Inspect KMS cryptographic activity | ✅ |
| **EBS Encryption** | EC2 storage | Protect root-volume data at rest | ✅ |
| **KMS Key State** | KMS | Control availability of cryptographic operations | ✅ |
 
 
---
 
 
## 🧠 Experiment 05 Security Flow
...
Create Customer Managed Key
   ...
   S3 SSE-KMS        EBS Encryption
 
        │               │
 
        ▼               ▼
 
Authenticated      Encrypted Root
 
Signed Access         Volume
...
Cryptographic Operation Blocked
 
      │
 
      ▼
 
Dependent Resource Operation Fails
...
Signed S3 Access Verified          ✅
 
CloudTrail KMS Events Inspected    ✅
 
Encrypted EBS Root Volume Created  ✅
 
KMS Disable Test Completed         ✅
...
| **AWS KMS** | Customer managed encryption keys and cryptographic operations |
| **Amazon EBS** | Encrypted block storage for EC2 |
| **AWS CloudTrail** | Audit KMS and AWS API activity |
...
| **05** | Encrypting Data at Rest by Using AWS KMS |
...
- Create and manage customer managed AWS KMS keys.
- Encrypt Amazon S3 objects using SSE-KMS.
- Understand authenticated access to KMS-protected S3 objects.
- Inspect KMS cryptographic operations using CloudTrail.
- Create encrypted EBS volumes from snapshots.
- Understand the effect of KMS key availability on dependent AWS resources.
- Apply encryption-at-rest controls as another layer of cloud defense.
